...
void Dispatcher::loop()
{
  ...
  { // radio has not been in Rx mode for 8 seconds!
    _err_flags |= ERR_EVENT_STARTRX_TIMEOUT;
  }

  if (outbound)
  { // waiting for outbound send to be completed
  ...
  uint32_t air_time;
  {
    ...
    if (len > 0)
    {
      ...
      else
      {
        ...
        if (sender_id == NODE_ID - 1 || sender_id == NODE_ID + 1)
        { // simulate that NODE_ID can only hear NODE_ID-1 or NODE_ID+1, eg. 3 can't hear 1
        }
        else
        {
...
void Dispatcher::checkSend()
{
  ...
  { // LBT - check if radio is currently mid-receive, or if channel activity
    if (cad_busy_start == 0)
    {

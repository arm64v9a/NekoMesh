                               ...
                               const uint8_t *data, bool is_flood)
{
  ...
  if (data[0] == 0)
  { // blank password, just check if sender is in ACL
  ...
  if (client == NULL)
  {
    ...
    { // keep number of FS writes to a minimum
                                   ...
                                   const uint8_t *data)
{
  if (anon_limiter.allow(rtc_clock.getCurrentTime()))
  {
    ...
    {                        // is this repeater currently disabled
...
int MyMesh::handleRequest(ClientInfo *sender, uint32_t sender_timestamp, uint8_t *payload, size_t payload_len)
{
  ...
  if (payload[0] == REQ_TYPE_GET_TELEMETRY_DATA)
  {
    ...
    { // Supported boards with built-in temperature sensor. ESP32-C3 may return NAN
                            ...
                            uint8_t *data, size_t len)
{
  ...
  { // received an initial request by a possible admin
    ...
    reply_path_len = -1;
    if (data[4] == 0 || data[4] >= ' ')
    { // is password, ie. a login request
                          ...
                          const uint8_t *app_data, size_t app_data_len)
{
  ...
  if (packet->path_len == 0 && !isShare(packet))
  {
    ...
    { // just keep neigbouring Repeaters
                            ...
                            uint8_t *data, size_t len)
{
  ...
  if (i < 0 || i >= acl.getNumClients())
  { // get from our known_clients table (sender SHOULD already be known
    // in this context)
    ...
    { // prevent replay attacks
      ...
      else
      {
        ...
        if (reply)
        {
          ...
          { // we have an out_path, so send DIRECT
...
#endif
  if (dp == reply)
  { // no neighbours, need empty response
    strcpy(dp, "-none-");
    dp += 6;
  }
...
void MyMesh::handleCommand(uint32_t sender_timestamp, char *command, char *reply)
{
  if (region_load_active)
  {
    ...
    {                        // empty/blank line, signal to terminate 'load' operation
  ...
  {                            // optional prefix (for companion radio CLI)
  ...
  { // format:  setperm {pubkey-hex} {permissions-int8}
    char *hex = &command[8];
...
void MyMesh::loop()
{
  ...
  {                   // apply pending (temporary) radio params

...
static int lfs_cache_read(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache, lfs_block_t block,
...
static int lfs_cache_cmp(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache, lfs_block_t block,
...
static int lfs_cache_crc(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache, lfs_block_t block,
...
static int lfs_cache_flush(lfs_t *lfs, lfs_cache_t *pcache, lfs_cache_t *rcache)
{
  if (pcache->block != 0xffffffff)
  {
    ...
    {
      int res =
          lfs_cache_cmp(lfs, rcache, NULL, pcache->block, pcache->off, pcache->buffer, lfs->cfg->prog_size);
      if (res < 0)
      {
        return res;
      }
...
static int lfs_dir_commit(lfs_t *lfs, lfs_dir_t *dir, const struct lfs_region *regions, int count)
{
  ...
  if (relocated)
  {
    ...
    LFS_DEBUG("Relocating %" PRIu32 " %" PRIu32 " to %" PRIu32 " %" PRIu32, oldpair[0], oldpair[1],
  ...
  nextname:
    ...
    *path = pathname;
    ...
    while (true)
    {
      ...
      int res =
          lfs_bd_cmp(lfs, dir->pair[0], entry->off + 4 + entry->d.elen + entry->d.alen, pathname, pathlen);
      if (res < 0)
      {
        return res;
      }
...
int lfs_dir_read(lfs_t *lfs, lfs_dir_t *dir, struct lfs_info *info)
{
  ...
  int err =
      lfs_bd_read(lfs, dir->pair[0], entry.off + 4 + entry.d.elen + entry.d.alen, info->name, entry.d.nlen);
  if (err)
  {
    return err;
  }
...
static int lfs_ctz_find(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache, lfs_block_t head,
...
static int lfs_ctz_traverse(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache, lfs_block_t head,
...
lfs_ssize_t lfs_file_read(lfs_t *lfs, lfs_file_t *file, void *buffer, lfs_size_t size)
{
  ...
  while (nsize > 0)
  {
    ...
    {
      int err =
          lfs_ctz_find(lfs, &file->cache, NULL, file->head, file->size, file->pos, &file->block, &file->off);
      if (err)
      {
        return err;
      }
  ...
  while (nsize > 0)
  {
    ...
    if (!(file->flags & LFS_F_WRITING) || file->off == lfs->cfg->block_size)
    {
      if (!(file->flags & LFS_F_WRITING) && file->pos > 0)
      {
        ...
        int err = lfs_ctz_find(lfs, &file->cache, NULL, file->head, file->size, file->pos - 1, &file->block,
                               &file->off);
        if (err)
        {
      ...
      lfs_alloc_ack(lfs);
      int err =
          lfs_ctz_extend(lfs, &lfs->rcache, &file->cache, file->block, file->pos, &file->block, &file->off);
      if (err)
      {
        file->flags |= LFS_F_ERRED;
        return err;
...
int lfs_stat(lfs_t *lfs, const char *path, struct lfs_info *info)
{
  ...
  {
    err =
        lfs_bd_read(lfs, cwd.pair[0], entry.off + 4 + entry.d.elen + entry.d.alen, info->name, entry.d.nlen);
    if (err)
    {
      return err;
    }
  ...
  if (true)
  {
    ...
    for (int i = 0; i < 2; i++)
    {
      err = lfs_dir_commit(lfs, &superdir,
                           (struct lfs_region[]){ { sizeof(superdir.d), sizeof(superblock.d), &superblock.d,
                                                    sizeof(superblock.d) } },
                           1);
      if (err && err != LFS_ERR_CORRUPT)
      {
